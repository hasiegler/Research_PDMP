...
income_data <- readxl::read_xlsx(here("MedianIncomesCleaned.xlsx"))
...
opioid_deaths_data <- opioid_deaths_data[,c(2, 4, 6, 7)]

opioid_deaths_data$Overdose_Rate <- opioid_deaths_data$Deaths / opioid_deaths_data$Population * 100000

opioid_deaths_data <- opioid_deaths_data %>% 
  select(-Deaths, -Population)


## HEROIN Synthetic T40.1 40.4

heroin_data <- read.delim(here("Heroin_Synthetic.txt"))
...
## OTHER OPIOIDS T40.2 40.6

other_opioids_data <- read.delim(here("Other.txt"))
other_opioids_data <- other_opioids_data[,c(2, 4, 6, 7)]

other_opioids_data$other_rate <- other_opioids_data$Deaths / other_opioids_data$Population * 100000

other_opioids_data <- other_opioids_data %>% 
  select(-Deaths, -Population)

##METHADONE T40.3

methadone_data <- read.delim(here("Methadone.txt"))
...
all <- left_join(income_data, opioid_deaths_data, by = c("Year", "State"))
all <- left_join(all, heroin_data, by = c("Year", "State"))
all <- left_join(all, other_opioids_data, by = c("Year", "State"))
all <- all %>% 
...
data_missing <- all %>% 
  ...
  summarise(miss_total = sum(is.na(Overdose_Rate)),
            ...
            miss_other = sum(is.na(other_rate)))

sum(data_missing$miss_other)

## INTERPOLATING 3 RATES

interpolated_data <- list()

for (state in unique(all$State)) {
  ...
  state_data$other_rate <- na_interpolation(state_data$other_rate, option = "linear")
...
#write.csv(interpolated_df, file = "OVERDOSE3.csv", row.names = FALSE)
...
sum(is.na(methadone$methadone_rate))
...
health_data <- read.csv(here("PersonalHealthCare.csv"))
...
poverty_data <- readxl::read_xlsx(here("PovertyRatesCleaned.xlsx"))
colnames(poverty_data) <- poverty_data[1,]
poverty_data <- poverty_data[-1:-52,]
poverty_data <- poverty_data[-2280:-2281,]
poverty_data <- poverty_data[,c(1,2,3,5)]
poverty_data <- poverty_data[-160:-212,]
poverty_data <- poverty_data[-372:-424,]
poverty_data <- poverty_data[-1167:-2173,]
poverty_data <- poverty_data %>% 
  filter(State != "State")
Year <- rep(c(2020:1999), each=52)
poverty_data <- poverty_data %>% 
  cbind(Year)
poverty_data <- poverty_data %>% 
  drop_na()
poverty_data <- poverty_data %>% 
  select(State, Year, `Percent in poverty`) %>% 
  rename(poverty_rate = "Percent in poverty")
write.csv(poverty_data, "POVERTY.csv", row.names = FALSE)
